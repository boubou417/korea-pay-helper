...
/** V6.2.2: complete each wallet, BACK to Pay Helper, then foreground-launch the next wallet. */
public final class UnifiedSyncController {
    ...
    public static synchronized void stop(Context c,boolean home){if(!running)return;running=false;stage=-1;scheduledRun=false;diagnostic="stopped";H.removeCallbacksAndMessages(TOKEN);try{GoogleWalletSyncControllerV3.stop(false);}catch(Throwable ignored){}PaymentSyncStateBridge.prepareHandoff(PayAccessibilityService.Companion.getInstance());if(home)returnHome(c);}
    private static void finishAtHome(PayAccessibilityService service){synchronized(UnifiedSyncController.class){if(!running)return;running=false;stage=4;diagnostic=scheduledRun?"scheduled completed; waiting import":"manual completed at Pay Helper";H.removeCallbacksAndMessages(TOKEN);}try{GoogleWalletSyncControllerV3.stop(false);}catch(Throwable ignored){}PaymentSyncStateBridge.prepareHandoff(service);}
