...
public class AutoCapturePlugin extends Plugin {
    ...
    private boolean accessibilityEnabled() {
        ...
        return enabled.contains(cn.flattenToString()) || enabled.contains(cn.flattenToShortString()) || enabled.contains(PayAccessibilityService.class.getName());
    }
    @PluginMethod public void getStatus(PluginCall call) {
        JSObject out=new JSObject(); out.put("accessibilityEnabled",accessibilityEnabled()); out.put("serviceConnected",service()!=null);
        out.put("jkoCount",JkosTransactionStore.INSTANCE.load(getContext()).size()); out.put("linePayCount",LinePayTransactionStore.INSTANCE.load(getContext()).size());
        out.put("piWalletCount",PiWalletTransactionStore.INSTANCE.load(getContext()).size()); out.put("googleWalletCount",GoogleWalletTransactionStore.INSTANCE.load(getContext()).size());
        out.put("googleWalletRunning",GoogleWalletSyncControllerV3.isRunning()); out.put("googleDiagnosticCount",GoogleWalletDiagnosticStore.INSTANCE.load(getContext()).size());
        out.put("unifiedSyncRunning",UnifiedSyncController.isRunning()); out.put("unifiedSyncStage",UnifiedSyncController.getStage());
        out.put("nightlySyncEnabled",true); out.put("nightlySyncHour",2); NightlySyncReceiver.scheduleNext(getContext()); call.resolve(out);
    }
