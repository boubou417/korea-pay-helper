...
data class LinePayDiagnosticCapture(
    val time: Long,
...
object LinePayDiagnosticStore {
    private const val PREF = "linepay_diagnostic_captures"
    private const val KEY = "items"
    private const val MAX = 80

    fun add(c: Context, x: LinePayDiagnosticCapture) {
    ...
    fun load(c: Context): List<LinePayDiagnosticCapture> = try {
        val a = JSONArray(c.getSharedPreferences(PREF, 0).getString(KEY, "[]") ?: "[]")
        (0 until a.length()).map { i ->
            val o=a.getJSONObject(i)
            LinePayDiagnosticCapture(
