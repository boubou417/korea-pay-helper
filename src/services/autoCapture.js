...
export const parseCaptureTime = (value) => {
  ...
  if (!m) { const d = new Date(text); return Number.isNaN(d.getTime()) ? Date.now() : d.getTime(); }
...
const readSettingsMap = () => {
  try { return JSON.parse(localStorage.getItem('settingsMap') || '{}'); } catch { return {}; }
};

const ruleActiveAt = (p, time) => {
  const d = new Date(time); if (Number.isNaN(d.getTime())) return true;
  const day = `${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())}`;
  return (!p.promoStart || day >= p.promoStart) && (!p.promoEnd || day <= p.promoEnd);
};

const findPaymentRule = (tx, payments, time) => {
  const wid = walletId(tx); if (!wid) return null;
  let candidates = payments.filter(p => Array.isArray(p.mobileWallets) && p.mobileWallets.includes(wid) && ruleActiveAt(p,time));
  const last4 = String(tx.cardLast4 || '').replace(/\D/g,'').slice(-4);
  if (last4) {
    const exact = candidates.filter(p => String(p.cardLast4 || '').replace(/\D/g,'').slice(-4) === last4);
    if (exact.length === 1) return exact[0];
    if (exact.length > 1) candidates = exact;
  }
  const bank = String(tx.bank || '').trim();
  if (bank) {
    const bankMatches = candidates.filter(p => [p.bankName,p.bankShortName].filter(Boolean).some(n => bank.includes(n) || String(n).includes(bank)));
    if (bankMatches.length === 1) return bankMatches[0];
    if (bankMatches.length > 1) candidates = bankMatches;
  }
...
export function mergeTransactionsIntoTaiwanHistory(transactions=[]) {
  ...
  const payments = Array.isArray(twSettings.payments) ? [...twSettings.payments] : [];
  ...
  transactions.forEach(tx => {
    ...
    const rule = findPaymentRule(tx,payments,time);
    ...
    historyMap.TW.push({
      ...
      sourceLabel,
      autoCaptureKey:key,
      paymentMethod:tx.paymentMethod||'', paymentAccount:tx.paymentAccount||'',
      bank:tx.bank||'', cardLast4:tx.cardLast4||'', transactionId:tx.transactionId||'', detailChecked:Boolean(tx.detailChecked),
      mobileWallet:wid,
    ...
    if (rule) {
      const idx = payments.indexOf(rule);
      if (idx >= 0) payments[idx] = {...payments[idx], used:Number(payments[idx].used||0)+amount};
      matched++;
    } else unmatched++;
    known.add(key); added++;
  });
  ...
  return {added,total:historyMap.TW.length,matched,unmatched};
}

export async function syncCapturedTransactionsNow({ refreshGoogleWallet=true }={}) {
  const before = await AutoCapture.getTransactions();
  const first = mergeTransactionsIntoTaiwanHistory(before.transactions||[]);
  if (refreshGoogleWallet) await AutoCapture.syncGoogleWallet();
  return first;
}
...
export async function exportGoogleWalletDiagnostics(captures=[]) {
  ...
  if (!Capacitor.isNativePlatform()) {
    const blob=new Blob([data],{type:'application/json;charset=utf-8'}); const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url;a.download=fileName;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000); return {fileName};
  }
