...
export default function AutoCapturePanel({darkMode,onSyncComplete}){
 ...
 const autoImport=async(showMessage=false)=>{if(!native||importingRef.current)return null;importingRef.current=true;try{
   ...
   if(finishedUnified){
     // The native chain deliberately returns through Pay Helper between wallets on
     // Android 16. Only the FINAL return changes the web UI: import once, then home.
     window.setTimeout(()=>{if(onSyncComplete)onSyncComplete(merged);else window.location.reload();},250);
   }else if(merged.added>0){window.setTimeout(()=>window.location.reload(),500);}
   return merged;
 }catch(error){if(showMessage)setMessage(`同步寫入失敗：${error?.message||error}`);return null;}finally{importingRef.current=false;}};
 useEffect(()=>{if(!native)return;refresh();ensureNightlySync().catch(console.warn);const onVisible=()=>{if(!document.hidden)autoImport(false);};const onFocus=()=>autoImport(false);document.addEventListener('visibilitychange',onVisible);window.addEventListener('focus',onFocus);const timer=window.setInterval(async()=>{if(document.hidden)return;const s=await refresh();if(s?.unifiedSyncRunning){wasUnifiedRunningRef.current=true;return;}if(wasUnifiedRunningRef.current)autoImport(false);},900);return()=>{document.removeEventListener('visibilitychange',onVisible);window.removeEventListener('focus',onFocus);window.clearInterval(timer);};},[native]);
 const instantSync=async()=>{setBusy(true);wasUnifiedRunningRef.current=true;setMessage('即時同步已啟動：街口 → LINE Pay → Pi 拍錢包 → Google Pay。四個來源完成後會一次寫入並回首頁。');try{const merged=await syncCapturedTransactionsNow();if(merged.added>0)setMessage(`已先整理 ${merged.added} 筆既有交易；現在開始更新四個支付來源。`);}catch(error){wasUnifiedRunningRef.current=false;const text=String(error?.message||error||'UNKNOWN');setMessage(text.includes('ACCESSIBILITY')?'請先開啟「Pay Helper 自動消費同步」無障礙服務。':text.includes('SYNC_ALREADY_RUNNING')?'目前已有同步正在進行中。':`即時同步啟動失敗：${text}`);}finally{setBusy(false);setTimeout(refresh,500);}};
 ...
 {status?.unifiedSyncRunning&&<div className={`${darkMode?'bg-blue-950':'bg-blue-50'} rounded-xl p-3 text-xs mb-3`}>同步進行中：{stages[status?.unifiedSyncStage]||'處理中'}。Pay Helper 只作為 Android 16 的快速接力站，不會在中途整理或重新載入。</div>}
 ...
 <div className={`${darkMode?'bg-gray-700/60':'bg-gray-50'} rounded-xl p-3 text-xs mb-3`}><b>每日自動同步：02:00</b><div className="opacity-70 mt-1">已自動排程。Android 省電/Doze 可能讓執行時間稍微延後；全部完成後會回 Pay Helper 首頁，再由手機原本的螢幕逾時自行熄屏。</div></div>
 ...
 {message&&<div className={`mt-3 text-xs leading-relaxed ${darkMode?'text-gray-300':'text-gray-600'}`}>{message}</div>}<div className="mt-3 text-[10px] opacity-50 leading-relaxed">穩定流程：支付 App 掃描完成後透過 Accessibility 返回 Pay Helper；Pay Helper 前景 Activity 立即接力啟動下一個支付。中途不匯入、不 reload；Google Pay 完成後才一次去重、配對優惠、寫入台灣消費紀錄並切回首頁。</div></div>;
}
