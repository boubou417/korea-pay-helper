...
export default function AdvisorCard({ payment, exchangeRate, currencySymbol, darkMode }) {
  ...
  const spendLimit = payment.spendLimit || (
    payment.bonusRate > 0 ? payment.bonusLimit / payment.bonusRate : Number.MAX_SAFE_INTEGER
  );
  const used = Number(payment.used || 0);
  const remainTwd = Math.max(spendLimit - used, 0);
  ...
  const usedPercent = Number.isFinite(spendLimit) && spendLimit > 0
    ? Math.min(100, Math.round((used / spendLimit) * 100))
    : 0;

  const reasons = [
    `目前可用回饋約 ${formatRate(totalRate)}`,
    `尚可使用約 ${currencySymbol}${remainLocal.toLocaleString()}`,
    usedPercent >= 80 ? '接近上限，建議優先使用' : '加碼空間仍充足'
  ];
    ...
    <SurfaceCard darkMode={darkMode}>
      ...
      <div className={`rounded-2xl p-3 ${darkMode ? 'bg-gray-700/70' : 'bg-blue-50'}`}>
        <div className="text-sm">
          今天建議先使用 <span className="font-black text-blue-500">{payment.name}</span>
        </div>
