...
export default function App() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (result.platform === 'web') alert(`已匯出備份：${result.fileName}`);
    } catch (err) {
  ...
  const setHistory = (updater) => {
    setHistoryMap(prev=>{
      ...
      const newList = typeof updater === 'function' ? updater(current) : updater;
  ...
  const applyStrategy = (steps) => {
    ...
    if (mode === 'cash') {
      setHistory(prev => [{ time: Date.now(), name: '現金', amount: Number(amount || 0), note, category }, ...prev]);
      setAmount('');
      setNote('');
      return;
    }
    ...
    setSettingsMap(prev => {
      ...
      return {
        ...
        [targetCountry]: {
          ...
          payments: recalcUsedByHistory(currentSettings.payments, currentHistory, newRate)
        }
      };
    });
  };
    ...
    setSettingsMap(prev=>{
      ...
      newPayments.forEach(p=>{
        if (p.resetDay && day > p.resetDay) {
          newLastReset[country] = { ...newLastReset[country], [p.name]: Date.now() };
        }
      });
      setLastResetMap(newLastReset);
      return { ...prev, [country]: { ...current, payments: newPayments } };
    });
  },[country, settingsMap, lastResetMap]);
